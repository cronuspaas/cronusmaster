...
//		Account account = new AccountFactory().setUsername(username)
//				.setPassword(password).setAuthUrl(authenticationUrl)
...
//	}
	...
	public synchronized static Account getSwiftClient() {
		if (account == null) {
			account = new AccountFactory().setUsername(username)
					.setPassword(password).setAuthUrl(authenticationUrl)
					.setTenantId(tenantId).setTenantName(tenantName)
					.createAccount();
		}
