...
public class PlayClassPathBeanDefinitionScanner extends ClassPathBeanDefinitionScanner 
{
  ...
  /**
   * The override, which searches through the play framework's classes, instead of using files (as
   * Spring is trained to do).
   */
  @Override
  public Set<BeanDefinition> findCandidateComponents(String basePackage) {
    Logger.info("Finding candidate components with base package: " + basePackage);
    ...
    try {
      for (ApplicationClass appClass : Play.classes.all()) {
        if (appClass.name.startsWith(basePackage)) {
          Logger.info("Scanning class: " + appClass.name);
          ...
          if (isCandidateComponent(metadataReader)) {
            ...
            if (isCandidateComponent(sbd)) {
              candidates.add(sbd);
              Logger.info("Found bean %s", sbd.getBeanClassName());
            }
          }
        } else {
